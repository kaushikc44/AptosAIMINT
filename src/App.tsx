...
function App() {
  ...
  const collectionMint = async () => {
    ...
    const txnHash = await (window as any).martian.createCollection(text, "IDKWHAISSADA", "https://aptos.dev")
    console.log("Printing the hash",txnHash);
    for (let i = 0; i < urlimages.length; i++) {
      const txnHashtoken = await (window as any).martian.createToken(text,  `${text}${i}`, "keep it simple", 1, urlimages[i], 1)
